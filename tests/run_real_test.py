...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('signing_test.log')
    ]
)
...
def test_real_signing():
    ...
    # Test with real IPA
    ipa_path = r"C:\Users\Admin\Downloads\SpotilifeC.v1.2.3_v8.6.42.ipa"
    ...
    try:
        ...
        if success:
            ...
            if os.path.exists(output_path):
                logger.info("\nVerifying signed IPA:")
                logger.info(f"- File exists: Yes")
                size = os.path.getsize(output_path)
                logger.info(f"- File size: {size:,} bytes")
                if size > 1000000:  # >1MB should be ok
                    logger.info("- Size check: Passed")
                else:
                    logger.warning("- Size check: Warning - file seems small")
            return True
        else:
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("Check signing_test.log for detailed output")
    else:
        print("\nTest failed!")
        print("Check signing_test.log for error details")
