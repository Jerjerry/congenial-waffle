...
def main():
    ...
    if os.path.exists(cert_path):
        valid, details = validator.validate_p12(cert_path, "password")
        print(f"\nCertificate validation: {'✓' if valid else '✗'}")
        print(details)
        
    # Test provisioning profile
    profile_path = "path/to/profile.mobileprovision"
    if os.path.exists(profile_path):
        valid, details = validator.validate_provisioning_profile(profile_path)
        print(f"\nProfile validation: {'✓' if valid else '✗'}")
        print(details)
        
    # Test compatibility
    if os.path.exists(cert_path) and os.path.exists(profile_path):
        valid, details = validator.check_cert_profile_compatibility(cert_path, profile_path)
        print(f"\nCompatibility check: {'✓' if valid else '✗'}")
        print(details)
